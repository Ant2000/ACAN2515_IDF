...
ACAN2515Settings::ACAN2515Settings(const uint32_t inQuartzFrequency,
    ...
    if (mDesiredBitRate != inWhishedBitRate) {
        ...
        //--- Set the BRP
        mBitRatePrescaler = static_cast<uint8_t>(bestBRP);
        //--- Compute PS2
        const uint32_t PS2 = (bestTQCount + 1) / 3; // Always 2 <= PS2 <= 8
        mPhaseSegment2 = static_cast<uint8_t>(PS2);
        ...
        mPhaseSegment1 = static_cast<uint8_t>(PS1);
        //--- Set PS to what is left
        mPropagationSegment = static_cast<uint8_t>(propSegmentPlusPhaseSegment1 - PS1); // Always 1 <= PropSeg <= 8
        ...
        constexpr uint64_t ppm = 1000UL * 1000UL; // UL suffix is required for Arduino Uno
        mBitRateClosedToDesiredRate = diff * ppm <= static_cast<uint64_t>(W) * inTolerancePPM;
    }
} ;
...
ACAN2515Settings::ACAN2515Settings(const uint32_t inQuartzFrequency, // In Hertz
    ...
    mQuartzFrequency(inQuartzFrequency),
    mDesiredBitRate(bitrateFrom(inQuartzFrequency, inBitRatePrescaler, inPropagationSegment, inPhaseSegment1,
                                inPhaseSegment2)),
...
uint32_t ACAN2515Settings::actualBitRate() const {
...
bool ACAN2515Settings::exactBitRate() const {
    const uint32_t TQCount = 1 /* Sync Seg */ + mPropagationSegment + mPhaseSegment1 + mPhaseSegment2;
...
uint32_t ACAN2515Settings::ppmFromDesiredBitRate() const {
    const uint32_t TQCount = 1 /* Sync Seg */ + mPropagationSegment + mPhaseSegment1 + mPhaseSegment2;
    ...
    constexpr uint64_t ppm = 1000UL * 1000UL; // UL suffix is required for Arduino Uno
    return static_cast<uint32_t>(diff * ppm / W);
}

//----------------------------------------------------------------------------------------

uint32_t ACAN2515Settings::samplePointFromBitStart() const {
    const uint32_t TQCount = 1 /* Sync Seg */ + mPropagationSegment + mPhaseSegment1 + mPhaseSegment2;
    const uint32_t samplePoint = 1 /* Sync Seg */ + mPropagationSegment + mPhaseSegment1 - mTripleSampling;
    constexpr uint32_t partPerCent = 100;
    return (samplePoint * partPerCent) / TQCount;
}

//----------------------------------------------------------------------------------------

uint16_t ACAN2515Settings::CANBitSettingConsistency() const {
